...
class GndDataService
{
    ...
    public function searchPersonRequest($searchTerm) {
        $response = Request::get($this->getApiUrl() . '/search?filter=type:Person&format=json&q=' . $this->searchTermReplacement($searchTerm))
            ->send();
    ...
    public function getPersonData($gndId) {
        $response = Request::get($this->getApiUrl() . '/' . $gndId . '.json')
            ->send();
    ...
    public function searchOrganisationRequest($searchTerm) {
        $response = Request::get($this->getApiUrl() . '/search?filter=type:CorporateBody&format=json&q=' . $this->searchTermReplacement($searchTerm))
            ->send();
    ...
    public function searchKeywordRequest($searchTerm) {
        $response = Request::get($this->getApiUrl() . '/search?filter=type:SubjectHeading&format=json:suggest&size=100&q=' . $this->searchTermReplacement($searchTerm))
            ->send();
